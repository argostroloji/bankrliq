// BANKRLIQ — x402 paid endpoint: pool-finder ($0.05 USDC per call)
// Deployed separately on x402.bankr.bot; the app iframe calls it with
// bankr.x402.fetch. Same server-script ABI as app scripts (injected globals).
//
// args: { chain: "base"|"robinhood", token0?, token1?, fee? }
//   - token0 + token1 → that exact pair, all (or one) fee tier(s)
//   - only token0 (CA search) → pairs the token with WETH + the chain stable
//   - neither → WETH/stable default
// Returns { chain, pools: [...] } with TVL, 24h volume (sampled from Swap
// logs), fees, APR, prices. Addresses chain-verified 2026-07-20.
